...
function installAgent(state, app, model) {
  ...
  try {
    ...
    execFileSync(path.join(staged, 'runtime/node'), ['--check', path.join(staged, 'src/platforms/macos/automatic.cjs')]);
    ...
    saveJson(plist, plistDefinition(path.join(agent, 'runtime/node'), path.join(agent, 'src/platforms/macos/automatic.cjs'), state));
