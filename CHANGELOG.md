...
- Preserve explicit user-data directories during package activation, and initialize missing identities when opening an otherwise current stopped copy.
...
- Windows patch revision 5 tolerates routine runtime, updater and appearance changes: validate official signatures and matching runtime versions instead of a per-build EXE/DLL hash list, recognize compatible updater interfaces structurally, and retain native appearance when cosmetic recipes no longer match. Fast request and policy checks remain mandatory. Native updates complete compatibility checks before asking the running app to quit.
