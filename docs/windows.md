...
The Windows implementation prepares a separate local Codex copy. Newer Owl runtimes require a Windows package identity, so the copy is registered locally as `CodexFast.Switch` and activated through that identity. WindowsApps and the official Store package remain unchanged. Use the stable **Codex Fast** Start menu shortcut or `launchers/windows/Open Codex Fast.cmd` for the patched copy. Official downloads, protocol handlers and file associations continue to belong to the original installation.

The one-click commands install the latest successfully tested main build, including the identity support described here; fixed v0.2.5 Release installers predate it. This is a local manifest registration using `Add-AppxPackage -Register`, not an external-location sparse package. Only one Fast installation can own this identity per Windows user. A second state directory is rejected while another installation owns it. Activation through `IApplicationActivationManager` preserves an explicit user-data directory. A stopped current generation also checks its identity before launch, even when no patch update is needed.
...
Locally registered copies also need a PRI resource index so Windows can select the official light/dark, target-size and unplated PNG variants. `identity.cjs` prepares these images and the bundled `package-icons.pri` before registration; unchanged resources are retained without rewriting them. The index contains 35 relative image paths and no artwork. Its readable input is `package-icons.json`; developers can regenerate it with `node scripts/build-windows-icons.cjs <absolute-path-to-makepri.exe>`. The generator validates the package name, paths and theme/size qualifiers. Users do not need the Windows SDK. If the official package removes an indexed variant, installation keeps the basic official logos without using an index that would point to a missing file.

Run a revision-7 `install.cmd` once to migrate older tools. Compatible future official builds can then use the same installed helper. Microsoft Store downloads remain separate from the local copy update. See [tested builds](tested-builds.md) for real update and request checks and their limitations.
...
Revision 7 adds the independent `compact-colors` transform when the full compact layout is unrecognized. It structurally matches the collapsed model/effort row and its reasoning selection, then uses the native `--color-chart-purple` for Ultra and `--color-text` for the model name and filled Fast icon. The icon is dark gray in light themes and light gray in dark themes. The transform preserves model, effort and service-tier selections, menu behavior and native spacing. It can retain these color fixes without recognizing the entire layout; an unknown or ambiguous color structure falls back separately to native colors without blocking an otherwise compatible core update. `compatibility.compactColors` records whether this color transform was applied.
...
The online **install.cmd** and **uninstall.cmd** are small downloaders. Every run reads the latest tested commit from the project's download site, fetches that commit's Windows package with bundled npm dependencies, verifies SHA-256 and the embedded build identity, validates archive paths and runs the extracted launcher. A concurrent deployment triggers one retry with fresh metadata. Download or verification failures leave the existing installation intact. Temporary downloads are removed afterward. `--help` works without network access. The launcher uses Node.js 22.12+ from Codex, the installed worker or the system; Git and npm are not needed by the online scripts.

The standalone scripts attached to numbered Releases still embed a fixed Windows package and verify its SHA-256 before extraction. Existing old scripts cannot update their own embedded payload: download the online script once to switch to checking the latest build on every run.

**install.cmd** runs `setup`: prepare or upgrade the local copy, install or update automatic monitoring, and open Codex. Setup and manual restart check compatibility before requesting Quit. A current copy is not restarted when the original app is closed. **uninstall.cmd** runs `uninstall`: request normal Quit for local copies, disable the monitor, remove the Startup entry, unregister the owned local package identity, delete all owned installation files, and return to the official app. The launcher uses a temporary Node copy during uninstall so an installed worker runtime does not prevent its own removal.
...
The README's fixed URLs use GitHub Pages at `https://infinityf4p.github.io/codex-fast-switch/`. A push to `main` builds both platform packages; after all six CI checks succeed, the download job publishes `latest.json`, ZIPs under the source commit, and the four online scripts. A build that has been superseded on main is not published. No Release, tag or version bump is needed. Updates to the Fast Switch tools are checked when an online installer is run; the existing background monitor still handles changes to the locally installed official app.

In an extracted Windows ZIP, the root `install.cmd` and `uninstall.cmd` run the bundled fixed version. The root scripts in a source checkout are the online downloaders; use `node cli.cjs setup` or `node cli.cjs uninstall` to run the local source after `npm ci --ignore-scripts`. Other scripts are in `launchers/windows` alongside their PowerShell runner. Packages include dependencies.
...
Restore does not copy an older version over a newer official update. **Restore Original App.cmd** unregisters the owned package identity and clears the local launch target while retaining local generations. **uninstall.cmd** validates generation records, unregisters matching local package entries before deleting their files, waits for the monitor to exit, and removes `versions`, `agent`, configuration, logs and the empty state directory. Unregistration is limited to the local package name and publisher at exact validated generation paths; the official package and registrations owned by another state are untouched. A failed removal stops before deleting copies or clearing their launch pointer, so the operation can be retried. Cleanup stays inside the recorded installation directory and rejects unexpected items or redirected paths. It does not depend on the active launch pointer being intact. The original app and its personal profile remain unchanged.

When switching generations, registration validates the old generation's ownership and requires its app to have exited. If new registration fails, it attempts to re-register the old manifest. The launcher also restores the previous active record if registration or shortcut setup fails. If activation itself reports an error after installation, the launch/update paths first confirm that related apps have stopped, then restore the previous record and identity before reopening the previous generation with its profile. Existing generation resources are used for recovery, so it does not require an older official source directory to still exist. Recovery errors remain visible alongside the initial failure. This handles activation-call errors, not a process that starts successfully and crashes later.
...
For builds requiring the local package identity, use a separate Windows user or VM when a personal Fast installation is already registered. A separate state/profile alone does not provide a second package identity; registration now refuses to replace the personal installation.
