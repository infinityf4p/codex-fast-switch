...
These appearance requirements remain mandatory on macOS. Windows revision 5 introduced native appearance fallback. Revision 6 retries from the original archive with only the incompatible appearance features disabled, preserving matching full model names and filled Fast glyphs; all Fast eligibility and request checks remain mandatory. Windows also accepts compatible updater interfaces through structural checks and verifies signed Owl runtime pairs without a per-build binary allowlist. See [Windows integrity and recovery](windows.md#integrity-and-recovery) for platform-specific checks and fallback behavior.
