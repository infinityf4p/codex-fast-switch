...
async function main() {
  ...
  if (command === 'restart') {
    ...
    let waiting = false;
    while (true) {
      const result = await automatic.restart(state, app, { model: values.model, onPhase: progress });
