...
async function main() {
  ...
  try {
    ...
    let events = '';
    observer = spawn(path.join(__dirname, '../../../build/macos/native-helper'), ['watch', app], { stdio: ['ignore', 'pipe', 'inherit'] });
    ...
    const result = await tx.withLock(state, () => restart(app, state, {
      ...
      quit: () => {
        const sent = execFileSync(path.join(__dirname, '../../../build/macos/native-helper'), ['quit', app], { encoding: 'utf8' });
