...
Windows patch revision 5 reuses the app's native update button, status and confirmation UI. Once the installed official app updates, the running Fast copy offers an update. Clicking it checks compatibility before quitting, then prepares a new patched copy and reopens it. A failed compatibility check leaves the app running; installation failures preserve the previous copy. Microsoft Store still downloads and installs the official app; the in-app update check can also open the Store.

Run an `install.cmd` containing revision 5 once to update existing tools. Routine updates no longer need a per-build runtime hash: the script checks OpenAI signatures, matching runtime versions and the updater interface. Changed cosmetic layouts fall back to the new native appearance while Fast behavior must still pass validation. An older tool's `Cannot identify ... ASAR integrity resource` error does not require reinstalling the official app or clearing personal data. Incompatible Fast request logic or updater interfaces can still require a newer script.
...
| Windows x64 | **26.911.61220 (9647)**, Store package **26.911.7940.0** | Revision 5 upgrade from an isolated 8881 copy through the native button, profile-preserving relaunch and automatic native appearance fallback; Fast/Standard requests passed |
