...
Windows 补丁 revision 5 继续复用 App 原生的更新按钮、提示和确认界面。本机官方原版更新后，运行中的 Fast 副本会显示更新提示；点击后先检查新版兼容性，通过后再正常退出、制作新版补丁副本并重新打开。兼容性检查失败时 App 保持运行，安装失败保留旧副本。官方原版的下载和安装仍由 Microsoft Store 负责，应用内检查更新也可打开商店。

已有安装先运行一次包含 revision 5 的 `install.cmd` 更新工具。后续常规更新不再要求逐版本添加运行时哈希：脚本检查 OpenAI 签名、运行时版本配对和更新器接口结构；界面样式发生变化时自动保留新版原生样式，Fast 功能仍必须通过校验。旧工具的 `Cannot identify ... ASAR integrity resource` 报错不需要重装官方 App 或清理个人数据。Fast 请求逻辑或更新接口发生不兼容改动时，仍可能需要新版脚本。
...
| Windows x64 | **26.911.61220 (9647)**，Store 包 **26.911.7940.0** | revision 5：从 8881 的隔离副本通过原生按钮升级、保留配置重启；自动使用新版原生样式，Fast/Standard 请求通过 |
