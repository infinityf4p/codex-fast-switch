...
async function planArchive(archive, recipes = JSON.parse(fs.readFileSync(path.join(__dirname, 'recipes.json'), 'utf8'))) {
  ...
  for (const entry of files) {
    ...
    if (!source.includes('fast_mode') && !source.includes('serviceTierForRequest')) continue;
    const result = adapt(source, recipes);
    if (!result.matches.length) continue;
    allMatches.push(...result.matches);
    targets.push({ entry, sourceSha256: sha256(bytes), patched: Buffer.from(result.patched),
      kinds: result.matches.map(item => item.recipe.kind) });
  }
...
module.exports = { parse, shape, at, nodePath, replacement, adapt, validateGates, planArchive };
