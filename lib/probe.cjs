...
function probeEnvironment(root, parent = process.env) {
  ...
  return { ...env, HOME: root, PATH: '/usr/bin:/bin:/usr/sbin:/sbin', TMPDIR: root,
...
async function discoverModel(app, root, requested) {
  const executable = path.join(app, 'Contents/Resources/codex');
  ...
  const child = spawn(executable, ['app-server', '--stdio'], {
    cwd: root, env: probeEnvironment(root), stdio: ['pipe', 'pipe', 'ignore'],
  });
