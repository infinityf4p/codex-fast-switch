...
const { healthCheck } = require('./health.cjs');
...
async function stage(app, destination, plan, metadata) {
  ...
  saveJson(path.join(destination, MARKER), { patchId: PATCH_ID, ...metadata,
  ...
  execFileSync('/usr/bin/codesign', ['--force', '--sign', '-', '--options', 'runtime',
    '--entitlements', path.join(__dirname, 'local-entitlements.plist'), destination], { stdio: 'pipe' });
  verify(destination);
}
...
async function install(app = discoverApp(), state = DEFAULT_STATE, { check = healthCheck, onPhase = () => {}, model } = {}) {
  app = fs.realpathSync(app);
  state = path.resolve(state);
  assertStopped(app);
  ...
  try {
    onPhase('staging');
    await stage(app, destination, plan, { id, ...appVersion });
    ...
    assertStopped(app);
    verify(app, true);
    if (!same(app, original)) throw new Error('Official app changed during verification. No update was overwritten.');
...
module.exports = { DEFAULT_STATE, PATCH_ID, MARKER, archivePath, fingerprint, same, saveJson,
