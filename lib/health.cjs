...
const { binaryPath } = require(process.platform === 'win32' ? '../windows/platform.cjs' : './platform.cjs');
...
async function healthCheck(app, { onProgress = () => {}, screenshot, model: requestedModel,
  ...
  try {
    ...
    cdp = windows ? await require('../windows/cdp.cjs').CdpSocket.connect(child, userData) : new CdpPipe(child);
